...
export const FloatingActionRail = ({
  ...
  return (
    <>
      {/* Collapse tab — shown when rail is open */}
      {open && (
        <Button
          size="icon"
          variant="outline"
          className="hidden xl:flex fixed top-24 right-[332px] z-40 h-7 w-7 rounded-full bg-white shadow-md"
      ...
      {/* Fixed rail on xl+ screens — only mounted when open */}
      {open && (
        <aside
          className="hidden xl:flex fixed top-20 right-4 w-[320px] max-h-[calc(100vh-6rem)] overflow-y-auto flex-col gap-3 z-30"
